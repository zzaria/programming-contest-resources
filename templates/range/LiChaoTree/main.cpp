...
const int N=P2(20),M=MOD;
ll n,K,dp[2][N],v[N],x,y;
ll f(int i,int x){
    return dp[0][i]+(v[i+1]-x)*(v[i+1]-x);
}
struct LiChaoTree{
    int seg[2*N];
    void reset(){
    ...
    void upd(int v,int l=0,int r=N,int x=1){
        int mid=(l+r)/2;
        if(f(seg[x],mid)>f(v,mid)) swap(seg[x],v);
        if(l==r) return;
        if(f(seg[x],l)>f(v,l)) upd(v,l,mid,2*x);
        else if(f(seg[x],r)>f(v,r)) upd(v,mid+1,r,2*x+1);
    }
    ll query(int x){
        ll ret=INF;
        for(int i=x+N;i;i/=2) ret=min(ret,f(seg[i],x));
        return ret;
    }
};
LiChaoTree lct;
...
int main(){
    ...
    cin>>n>>K;
    fori(1,n+1) cin>>v[i];
    seto(dp,inf); dp[0][0]=0;
    fori(1,K+1){
        forj(1,n+1){
            dp[1][j]=lct.query(v[j]);
            lct.upd(j);
        }
        lct.reset();
        swap(dp[0],dp[1]);
...
/**
dp[i][j]=dp[i-1][k-1]+v[k]^2+2*v[k]*v[j]+v[j]^2
*/
