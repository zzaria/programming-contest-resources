...
const int N=P2(18),M=1e9+7,L=30;
struct wavelet{
    const int MAXV=P2(L);
    vector<int> tol[N*L];
    int sz=1,lc[N*L],rc[N*L];
    void build(vector<int> a,int ind=0,int l=0,int r=MAXV-1){
        if(l==r||!sz(a)){
            sz--;
            return;
        }
        ...
        lc[ind]=sz++;
        build(A[0],lc[ind],l,mid);
        rc[ind]=sz++;
        build(A[1],rc[ind],mid+1,r);
    }
    int query(int lo,int hi,int k,int ind=0,int l=0,int r=MAXV-1){ //kth min number in lo to hi-1 0 indexed
        if(l==r) return l;
        int mid=(l+r)/2,temp=tol[ind][hi]-tol[ind][lo];
        if(temp>=k) return query(tol[ind][lo],tol[ind][hi],k,lc[ind],l,mid);
        return query(lo-tol[ind][lo],hi-tol[ind][hi],k-temp,rc[ind],mid+1,r);
    }
};
wavelet st;
int n,q,a,b,c;
...
int main(){
    ...
    cin>>n>>q; num.resize(n);
    fori(0,n) cin>>num[i];
    a=0;
    st.build(num);
    fori(0,q){
        cin>>a>>b>>c;
        cout<<st.query(a,b,c+1)<<"\n";
    }
    return 0;
}
/**
