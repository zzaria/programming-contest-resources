...
const int N=P2(16),L=16,M=1e9+7;
template<class T> struct SegTable{
    T st[L][N];
    T comb(T a,T b){return a+b;}
    void init(T* arr){
        fori(0,N) st[0][i]=arr[i];
        fori(1,L) for(int shift=P2(i),j=shift;j<N;j+=2*shift){
            st[i][j]=st[0][j]; st[i][j-1]=st[0][j-1];
            ford(k,j-2,j-shift) st[i][k]=comb(st[0][k],st[i][k+1]);
            fork(j+1,j+shift) st[i][k]=comb(st[i][k-1],st[0][k]);
        }
    }
    T query(int l,int r){
        if(l==r) return st[0][l];
        int lvl=31-clz(l^r);
        return comb(st[lvl][l],st[lvl][r]);
    }
};
