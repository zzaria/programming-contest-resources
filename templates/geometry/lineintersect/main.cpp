...
P lineIsect(Line &a,Line &b){
    T x=cross(a.pf,b.pf,a.ps),y=cross(a.pf,b.ps,a.ps);
    if(eq(x,y)) return P(sgn(x)==0? inf:-inf,inf);
    P xx=(x*b.ps-y*b.pf);
    return (x*b.ps-y*b.pf)/(x-y);
}
vector<P> segIsect(Line &a,Line &b){
    P x=lineIsect(a,b);
    set<P> s;
    if(x.ps==inf){
        if(onSeg(a.pf,b)) s.em(a.pf);
        if(onSeg(a.ps,b)) s.em(a.ps);
        if(onSeg(b.pf,a)) s.em(b.pf);
        if(onSeg(b.ps,a)) s.em(b.ps);
    }
    else if(onSeg(x,a)&&onSeg(x,b)) s.em(x);
    return {all(s)};
}
