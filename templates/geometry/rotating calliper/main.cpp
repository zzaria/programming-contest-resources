...
bool neq(const T& a,const T& b){return sgn(a-b);}
bool eq(const T& a,const T& b){return !sgn(a-b);}
bool lt(const T& a,const T& b){return sgn(a-b)<0;}
bool le(const T& a,const T& b){return sgn(a-b)<=0;}
bool gt(const T& a,const T& b){return sgn(a-b)>0;}
bool ge(const T& a,const T& b){return sgn(a-b)>=0;}
T sq(T a){return a*a;}
using P=pair<T,T>;
bool operator ==(const P& a,const P& b) {return eq(a.pf,b.pf)&&eq(a.ps,b.ps);}
bool operator <(const P& a,const P& b) {return neq(a.pf,b.pf)? lt(a.pf,b.pf): lt(a.ps,b.ps);}
bool operator <=(const P& a,const P& b) {return neq(a.pf,b.pf)? lt(a.pf,b.pf): le(a.ps,b.ps);}
bool operator >(const P& a,const P& b) {return neq(a.pf,b.pf)? gt(a.pf,b.pf): gt(a.ps,b.ps);}
bool operator >=(const P& a,const P& b) {return neq(a.pf,b.pf)? gt(a.pf,b.pf): ge(a.ps,b.ps);}
T norm(const P& p) { return sq(p.pf)+sq(p.ps); }
lld abs(const P& p) { return sqrt(norm(p)); } //probably shouldn't be integer
T arg(const P& p) { return atan2(p.ps,p.pf); }
P conj(const P& p) { return P(p.pf,-p.ps); }
P perp(const P& p) { return P(-p.ps,p.pf); }
P dir(T ang) { return P(cos(ang),sin(ang)); }
P operator *(const P& a,const P& b){return P(a.pf*b.pf-a.ps*b.ps,a.ps*b.pf+a.pf*b.ps);}
P operator /(const P& a,const P& b){return a*conj(b)/norm(b);}
P& operator *(P& a,const P& b){return a=a*b;}
P& operator /(P& a,const P& b){return a=a/b;}
P unit(const P& p) { return p/abs(p); }
T dot(const P& a, const P& b) { return a.pf*b.pf+a.ps*b.ps; }
T cross(const P& a, const P& b) { return a.pf*b.ps-a.ps*b.pf; }
T cross(const P& p, const P& a, const P& b) {return cross(a-p,b-p); }
T ang(const P &a,const P &b){return atan2(cross(a,b),dot(a,b));}//counterclockwise angle a to b
using Line=pair<P,P>;
P reflect(P p,Line l) { return l.pf+conj((p-l.pf)/(l.ps-l.pf))*(l.ps-l.pf); }
P foot(P p, Line l) { return (p+reflect(p,l))/(T)2;}
P rot(P a,T theta){return a*dir(theta);}
P rot(P a,T theta,P p){return rot(a-p,theta)+p;}
bool onSeg(P p, Line l) {return cross(l.pf,l.ps,p) == 0 && dot(p-l.pf,p-l.ps) <= 0;}
...
T area(vector<P> p){
    T ret=0;
    int n=sz(p);
    fori(0,n) ret+=cross(p[i],p[(i+1)%n]);
    return ret/2;
}
int n,m,a,b,c,j;
lld ans;
...
int main(){
    ...
    p=convexHull(p); n=sz(p); a=b=c=1; ans=INF;
    fori(0,n){
        j=(i+1)%n;
        if(a==i) a=j;
        while(cross(p[j]-p[i],p[(b+1)%n]-p[b])>0) b=(b+1)%n;
        while(lt(ang2(p[j]-p[i],p[(a+1)%n]-p[a]),PI/2)) a=(a+1)%n;
        while(lt(ang2(p[j]-p[i],p[(c+1)%n]-p[c]),PI*3/2)) c=(c+1)%n;
        dbg(p[i],p[b],p[a],p[c]);
        Line l=mp(p[i],p[j]);
        ckmin(ans,abs(foot(p[b],l)-p[b])*abs(foot(p[c],l)-foot(p[a],l)));
    }
    cout<<ll(round(ans));
    return 0;
}
