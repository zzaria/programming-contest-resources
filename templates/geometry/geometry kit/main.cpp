...
lld abs(const P& p) { return sqrt(norm(p)); }
lld arg(const P& p) { return atan2(p.ps,p.pf); }
...
lld ang(const P &a,const P &b){return atan2(cross(a,b),dot(a,b));}//counterclockwise angle a to b
using Line=pair<P,P>;
...
bool onSeg(P p, Line l) {return sgn(cross(l.pf,l.ps,p)/max(abs(l.pf-l.ps),abs(l.pf-p)))==0 && sgn(dot(p-l.pf,p-l.ps)) <= 0;}
