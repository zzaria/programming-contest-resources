...
struct hashmap{
    const static int SIZE=4500007;
    int key[SIZE];
    int val[SIZE];
    int to[SIZE],nxt[SIZE],vis[SIZE],sz=0,null=1;
    ll chash(ll x){x+=0x9e3779b97f4a7c15; x=(x^(x>>30))*0xbf58476d1ce4e5b9; x=(x^(x>>27))*0x94d049bb133111eb; return x^(x>>31)+SEED;}
    void clear(){
        sz=0; null++;
    }
    int upd(ll x,int v){
        int cur=(chash(x)%SIZE+SIZE)%SIZE,cur2=cur;
        if(vis[cur]!=null){vis[cur]=null; to[cur]=-1;}
        for(cur=to[cur];cur!=-1;cur=nxt[cur]) if(key[cur]==x){
            return val[cur]=v;
        }
        cur=cur2; nxt[sz]=to[cur]; to[cur]=sz; val[sz]=v; key[sz]=x;
        return val[sz++];
    }
};
