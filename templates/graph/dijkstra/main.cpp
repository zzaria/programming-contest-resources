...
int main(){
    ...
    toV.emplace(0,1); seto(dist,inf); dist[1]=0;
    while(sz(toV)){
        tie(w,v)=toV.top(); toV.pop();
        if(-w>dist[v]) continue;
        for(auto i:gr[v]) if(dist[i.pf]>dist[v]+i.ps){
            dist[i.pf]=dist[v]+i.ps;
            toV.emplace(-dist[i.pf],i.pf);
        }
    }
    fori(1,n+1) cout<<(dist[i]==inf? -1: dist[i])<<"\n";
    return 0;
}
/**
