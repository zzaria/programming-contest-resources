...
int main(){
    ...
    cin>>n>>m;
    fori(0,m){
        cin>>a>>b>>c; gr[a].eb(b,c);
    }
    foru(s,1,n+1){
        a=N*N; b=a-1;
        seto(dist,inf); dist[s]=0; toV[++b]=s; in[s]=1; seto(f,0);
        while(a<=b){
            int v=toV[a++]; in[v]=0; //assert(f[v]<n);
            if(++f[v]>n) dist[v]=-INF;
            for(auto i:gr[v]) if(dist[i.pf]>max(-INF/2,dist[v]+i.ps)){
                if(!in[i.pf]) a<=b&&dist[i.pf]<dist[toV[a]]?toV[--a]=i.pf:toV[++b]=i.pf;
                dist[i.pf]=dist[v]+i.ps; in[i.pf]=1;
                //for(sum+=dist[i.pf];sz(toV)&&sz(toV)*dist[toV.front()]>sum;toV.pop_front()) toV.push_back(toV.front());
            }
        }
        fori(1,n+1){
            if(dist[i]==INF) cout<<"INF";
            else if(dist[i]<=-INF/2) cout<<"-INF";
            else cout<<dist[i];
            if(i!=n) cout<<" ";
        }
        cout<<nl;
