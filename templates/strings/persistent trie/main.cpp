...
void ins(int num,int inx){
    ...
    sz++;
    tri[sz]=tri[tri[num].ch[in[inx]-'a']];
    tri[num].ch[in[inx]-'a']=sz;
    ins(sz,inx+1);
}
int sum(int num,int inx){
    if(!num) return 0;
    if(inx==in.size()) return tri[num].sz;
    return sum(tri[num].ch[in[inx]-'a'],inx+1)+tri[num].n;
}
...
int main(){
    ...
    cin>>n>>m;
    fori(0,n){
        cin>>in; rt[i+1]=++sz; tri[sz]=tri[rt[i]];
        ins(sz,0);
    }
    fori(0,m){
        cin>>in>>l>>r;
        cout<<sum(rt[r],0)-sum(rt[l-1],0)<<"\n";
    }
    return 0;
}
/**
